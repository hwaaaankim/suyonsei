...
(function ($) {
  ...
  $(document).ready(function () {
    ...
    $(document).on("click", ".navbar-nav a.move-page", function (e) {
      e.preventDefault();
      location.href = $(this).attr("href");
    });
    //Hide Alert Box
    ...
    if ($("#mycountdown").length > 0) {
      $("#mycountdown").countdown("2019/11/23", function (event) {
    ...
    if (surgerySliderTwo.length > 0) {
      ...
      surgerySliderTwo.on(
        ...
        function (event, slick, currentSlide, nextSlide) {
          ...
          $(".slider-arrow-area .arrow-top").text(
            check_number(currentSliderIndex)
          );
          ...
          var nextSliderTitle = $(
            '.surgery-slider-wrapper .slick-slide[data-slick-index="' +
              currentSliderIndex +
              '"]'
          )
            .find(".slider-heading")
          ...
          var nextSliderimg = $(
            ...
            .find(".slider-img-area")
            .css("background-image");

          $(".surgery-slider-area-02 .slider-controller .left-controller").css(
            "background-image",
            nextSliderimg
          );
          $(".surgery-slider-area-02 .slider-controller .right-controller")
            .find(".heading-03")
            .text(nextSliderTitle);
        }
      );
    }
    ...
    if (timelineSliderBig.length > 0) {
      ...
      timelineSliderBig.on(
        "beforeChange",
        function (event, slick, currentSlide, nextSlide) {
          ++nextSlide;
          $(".big-slider-area .year-area ul li").removeClass("active");
          $(
            ".big-slider-area .year-area ul li:nth-child(" + nextSlide + ")"
          ).addClass("active");
        }
      );
    }
    ...
    if (dentalSlider.length > 0) {
      dentalSlider.slick({
        ...
        arrows: true,
        prevArrow:
          '<div class="slick-prev">  <div class="arrow-down"> <span class="arrow-line"></span><span class="caret-down"></span></div></div>',
        nextArrow:
          '<div class="slick-next"> <div class="arrow-up"> <span class="arrow-line"></span><span class="caret-up"></span></div> </div>',
      });
      ...
      dentalSlider.on(
        ...
        function (event, slick, currentSlide, nextSlide) {
          ...
          var progressWidth = progress * currentSliderIndex;
          SliderProgressbarStart(progressWidth);
          $(".controller-area .total-controller").text(
            check_number(totalSlide)
          );
          $(".controller-area .active-controller").text(
            check_number(currentSliderIndex)
          );
        }
      );
    ...
    if (testimonialCarouselImg.length > 0) {
      ...
      testimonialCarouselImg.on(
        ...
        function (event, slick, currentSlide, nextSlide) {
          ...
          $(".testimonials-carousel-controller .total-controller").text(
            check_number(totalSlide)
          );
          $(".testimonials-carousel-controller .active-controller").text(
            check_number(currentSliderIndex)
          );
          if (nextSlide > 0) {
            var customActive = nextSlide + 2;
            testimonialCarouselImg
              .find(".slick-slide")
              .removeClass("right_active_item");
            testimonialCarouselImg
              .find('.slick-slide[data-slick-index="' + customActive + '"]')
              .addClass("right_active_item");
          } else {
            testimonialCarouselImg
              .find(".slick-slide")
              .removeClass("right_active_item");
            testimonialCarouselImg
              .find('.slick-slide[data-slick-index="4"]')
              .addClass("right_active_item");
          }
        }
      );
    ...
    if (PlasticSurgeryOne.length > 0) {
      ...
      PlasticSurgeryOne.on(
        ...
        function (event, slick, currentSlide, nextSlide) {
          ...
          $(".controller-wrapper .total-controller").text(
            check_number(totalSlide)
          );
          $(".controller-wrapper .active-controller").text(
            check_number(currentSliderIndex)
          );
        }
      );
    ...
    if (dentalProgressBar.length > 0) {
      dentalProgressBar.rProgressbar({
        percentage: 90,
        fillBackgroundColor: "#F2F5F7",
        backgroundColor: "transparent",
        AbsoluteProgressCount: true,
      });
    }
    var cardioProgressBar = $(".cardio");
    if (cardioProgressBar.length > 0) {
      cardioProgressBar.rProgressbar({
        percentage: 70,
        fillBackgroundColor: "#F2F5F7",
        backgroundColor: "transparent",
        AbsoluteProgressCount: true,
    ...
    if (neurologieProgressBar.length > 0) {
      neurologieProgressBar.rProgressbar({
        percentage: 85,
        fillBackgroundColor: "#F2F5F7",
        backgroundColor: "transparent",
        AbsoluteProgressCount: true,
    ...
    if (orthopedyProgressBar.length > 0) {
      orthopedyProgressBar.rProgressbar({
        percentage: 65,
        fillBackgroundColor: "#F2F5F7",
        backgroundColor: "transparent",
        AbsoluteProgressCount: true,
  ...
  function testProgressbarStart(progressWidth) {
    $(
      ".testimonials-carousel-controller .testimonial-progressbar .progress-active-line"
